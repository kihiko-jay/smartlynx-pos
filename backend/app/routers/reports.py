...
def _build_z_tape_payload(txns, target_date, sid, store, include_timestamp=False):
    ...
    filtered = [
        t for t in txns
        if utc_to_merchant_date(t.completed_at, store.timezone) == target_date
    ]
    
    gross = sum(t.total_amount for t in filtered) if filtered else 0
    vat = sum(t.tax_amount for t in filtered) if filtered else 0
...
def _build_weekly_payload(txns, start_date, end_date, sid):
